...
class WTrackTableView : public WLibraryTableView {
  ...
  public:
    ...
    // IDs for the rows around the current selection, in current view order.
    QList<TrackId> getBrowseTrackIds(int radius) const;
    int getBrowseSelectedRow() const;
    int getBrowseRowCount() const;
