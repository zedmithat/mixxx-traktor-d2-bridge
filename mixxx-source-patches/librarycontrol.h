...
class LibraryControl : public QObject {
  ...
  private slots:
    ...
    // Expose the actual row selected by controller browsing. This is read-only
    // for scripts and lets display controllers show the library selection.
    void updateSelectedTrackId();
  ...
  private:
    ...
    std::unique_ptr<ControlObject> m_pSelectedTrackId;
    std::unique_ptr<ControlPushButton> m_pD2BrowseRefresh;
    // D2 navigation is intentionally independent of QApplication focus. A
    // controller must keep browsing when another window owns keyboard focus.
    std::unique_ptr<ControlPushButton> m_pD2TrackUp;
    std::unique_ptr<ControlPushButton> m_pD2TrackDown;
