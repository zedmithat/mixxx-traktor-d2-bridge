...
QStringList WLibrarySidebar::d2VisibleLabels(int radius) {
    ...
    int selectedRow = indexes.indexOf(selectedIndex());
    if (selectedRow < 0) {
        selectedRow = 0;
    }
    const int indexCount = static_cast<int>(indexes.size());
    const int windowSize = radius * 2 + 1;
    const int lastStart = std::max(0, indexCount - windowSize);
    const int windowStart = std::min(
            std::max(0, selectedRow - radius), lastStart);
    for (int offset = 0; offset < windowSize; ++offset) {
        const int row = windowStart + offset;
        labels.append(row >= 0 && row < indexes.size()
                ? indexes.at(row).data(Qt::DisplayRole).toString()
                : QString());
    }
    return labels;
}

int WLibrarySidebar::d2SelectedVisibleRow() {
    if (!model()) {
        return -1;
    }
...
int WLibrarySidebar::d2VisibleRowCount() {
    if (!model()) {
        return 0;
    }
    QModelIndex index = model()->index(0, 0, QModelIndex());
