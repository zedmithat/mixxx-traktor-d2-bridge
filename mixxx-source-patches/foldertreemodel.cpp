...
namespace {
bool shouldHideDjSystemDirectory(const QString& name) {
...
bool FolderTreeModel::directoryHasChildren(const QString& path) const {
    ...
    if (directory != nullptr) {
        ...
        while (!has_children && ((entry = readdir(directory)) != nullptr)) {
            if (entry->d_name != dot && entry->d_name != dotdot) {
                const QString entryName = QFile::decodeName(entry->d_name);
                if (shouldHideDjSystemDirectory(entryName)) {
                    continue;
                }
                total_count++;
    ...
    if (directory == nullptr || (unknown_count == total_count && total_count > 0)) {
        ...
        for (const QString& name : all) {
            if (!shouldHideDjSystemDirectory(name)) {
                has_children = true;
                break;
            }
        }
