...
LibraryControl::LibraryControl(Library* pLibrary)
    ...
    // Numeric, read-only mirror of the highlighted library row. It is kept
    // separate from MoveVertical so controller scripts can observe it without
    // affecting normal library navigation.
    m_pSelectedTrackId = std::make_unique<ControlObject>(
            ConfigKey("[Library]", "selected_track_id"));
    m_pD2BrowseRefresh = std::make_unique<ControlPushButton>(
            ConfigKey("[Library]", "d2_browse_refresh"));
    connect(m_pD2BrowseRefresh.get(),
    ...
    m_pD2TrackUp = std::make_unique<ControlPushButton>(
            ConfigKey("[Library]", "d2_track_up"));
    m_pD2TrackDown = std::make_unique<ControlPushButton>(
            ConfigKey("[Library]", "d2_track_down"));
    auto moveD2Track = [this](int delta) {
        if (!m_pLibraryWidget) {
            return;
        }
        WTrackTableView* pTrackTableView =
                m_pLibraryWidget->getCurrentTrackTableView();
        if (!pTrackTableView || pTrackTableView->getBrowseRowCount() <= 0) {
            scheduleD2BrowseUpdate(60);
            return;
        }
        pTrackTableView->moveSelection(delta);
        scheduleD2BrowseUpdate();
    };
    connect(m_pD2TrackUp.get(),
            &ControlPushButton::valueChanged,
            this,
            [moveD2Track](double value) {
                if (value > 0) {
                    moveD2Track(-1);
                }
            });
    connect(m_pD2TrackDown.get(),
            &ControlPushButton::valueChanged,
            this,
            [moveD2Track](double value) {
                if (value > 0) {
    ...
    m_pD2SidebarUp = std::make_unique<ControlPushButton>(
            ConfigKey("[Library]", "d2_sidebar_up"));
    m_pD2SidebarDown = std::make_unique<ControlPushButton>(
            ConfigKey("[Library]", "d2_sidebar_down"));
    connect(m_pD2SidebarUp.get(),
            &ControlPushButton::valueChanged,
            this,
            [this](double value) {
                if (value > 0) {
    ...
    for (std::size_t i = 0; i < m_pBrowseTrackIds.size(); ++i) {
        m_pBrowseTrackIds[i] = std::make_unique<ControlObject>(
                ConfigKey("[Library]", QStringLiteral("browse_track_id_%1").arg(i)));
    }
#ifdef MIXXX_USE_QML
...
void LibraryControl::bindSidebarWidget(WLibrarySidebar* pSidebarWidget) {
    ...
    QTimer::singleShot(0, this, &LibraryControl::writeD2BrowseState);
}
...
void LibraryControl::bindLibraryWidget(WLibrary* pLibraryWidget, KeyboardEventFilter* pKeyboard) {
    ...
    QTimer::singleShot(0, this, &LibraryControl::updateSelectedTrackId);
}
...
void LibraryControl::slotMoveVertical(double v) {
    ...
    emitKeyEvent(QKeyEvent{
            QEvent::KeyPress, key, Qt::NoModifier, QString(), false, times});
    // QTableView applies the key event synchronously, but defer one event-loop
    // cycle to cover model/proxy selection updates before reading its TrackId.
    scheduleD2BrowseUpdate();
}

void LibraryControl::scheduleD2BrowseUpdate(int delayMs) {
    // USB folders can emit a burst of selection/model notifications for one
    // encoder turn. Export only the settled state: controller navigation stays
    // synchronous, while the display IPC and temporary-state-file writes are
    // coalesced to at most one update per short interval.
    if (m_d2BrowseUpdatePending) {
        return;
    }
    m_d2BrowseUpdatePending = true;
    QTimer::singleShot(delayMs, this, [this] {
        m_d2BrowseUpdatePending = false;
        updateSelectedTrackId();
    });
}

void LibraryControl::updateSelectedTrackId() {
    if (!m_pLibraryWidget) {
        return;
    }
    WTrackTableView* pTrackTableView = m_pLibraryWidget->getCurrentTrackTableView();
    if (!pTrackTableView) {
        for (const auto& pControl : m_pBrowseTrackIds) {
            pControl->setAndConfirm(0.0);
        }
        m_pSelectedTrackId->setAndConfirm(0.0);
        writeD2BrowseState();
        return;
    }
    // A newly opened USB folder may populate its proxy model asynchronously and
    // initially has no current row. Select the first real track as soon as it is
    // available so the D2 gets a complete window without requiring an extra turn.
    if (pTrackTableView->getBrowseSelectedRow() < 0 &&
            pTrackTableView->getBrowseRowCount() > 0) {
        pTrackTableView->moveSelection(1);
    }
    const QList<TrackId> browseTrackIds = pTrackTableView->getBrowseTrackIds(4);
    for (std::size_t i = 0; i < m_pBrowseTrackIds.size(); ++i) {
        const TrackId trackId = browseTrackIds.value(static_cast<int>(i));
        m_pBrowseTrackIds[i]->setAndConfirm(
                trackId.isValid() ? trackId.toVariant().toDouble() : 0.0);
    }
    const TrackId trackId = pTrackTableView->getCurrentTrackId();
    m_pSelectedTrackId->setAndConfirm(
            trackId.isValid() ? trackId.toVariant().toDouble() : 0.0);
    writeD2BrowseState();
}

void LibraryControl::writeD2BrowseState() {
    if (!m_pLibraryWidget || !m_pSidebarWidget) {
        return;
    }
    WTrackTableView* pTrackTableView = m_pLibraryWidget->getCurrentTrackTableView();
    const int trackRow = pTrackTableView ? pTrackTableView->getBrowseSelectedRow() : -1;
    ...
    const int sidebarWindowStart = std::min(
            std::max(0, sidebarRow - 4), std::max(0, sidebarCount - 9));
    const int trackWindowRow = trackRow >= 0 ? trackRow - trackWindowStart : -1;
    const int sidebarWindowRow = sidebarRow >= 0 ? sidebarRow - sidebarWindowStart : -1;
    const QStringList sidebarLabels = m_pSidebarWidget->d2VisibleLabels(4);
    auto clean = [](QString text) {
        text.replace('\t', ' ');
        text.replace('\r', ' ');
        text.replace('\n', ' ');
        return text;
    ...
    QSaveFile file(QStringLiteral("/tmp/mixxx-d2-library-state"));
    ...
    for (int i = 0; i < 9; ++i) {
        stream << "ITEM" << i << '\t' << clean(sidebarLabels.value(i)) << '\n';
    }
    stream.flush();
    file.commit();
}
...
void LibraryControl::slotGoToItem(double v) {
    ...
    case FocusWidget::Sidebar:
        ...
        // Filesystem/USB features update through an asynchronous proxy model.
        // Capture both the fast path and the settled model, including slow media.
        QTimer::singleShot(220, this, &LibraryControl::updateSelectedTrackId);
        QTimer::singleShot(650, this, &LibraryControl::updateSelectedTrackId);
        return;
    case FocusWidget::TracksTable: {
