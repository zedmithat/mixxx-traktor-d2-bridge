...
QList<TrackId> WTrackTableView::getBrowseTrackIds(int radius) const {
    QList<TrackId> trackIds;
    TrackModel* pTrackModel = getTrackModel();
    QItemSelectionModel* pSelectionModel = selectionModel();
    if (!pTrackModel || !pSelectionModel || radius < 0) {
        return trackIds;
    }
    const QModelIndex current = pSelectionModel->currentIndex();
    QAbstractItemModel* pModel = model();
    if (!current.isValid() || !pModel) {
        return trackIds;
    }
    const int rowCount = pModel->rowCount();
    const int windowSize = radius * 2 + 1;
    const int lastStart = std::max(0, rowCount - windowSize);
    const int windowStart = std::min(
            std::max(0, current.row() - radius), lastStart);
    for (int offset = 0; offset < windowSize; ++offset) {
        const int row = windowStart + offset;
        if (row >= 0 && row < pModel->rowCount()) {
            trackIds.append(pTrackModel->getTrackId(
                    pModel->index(row, current.column())));
        } else {
            trackIds.append(TrackId());
        }
    }
...
int WTrackTableView::getBrowseSelectedRow() const {
...
int WTrackTableView::getBrowseRowCount() const {
