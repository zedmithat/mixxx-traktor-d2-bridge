...
bool shouldHideDjSystemEntry(const QString& name) {
    ...
    return hiddenNames.contains(normalized);
}

bool isLikelyAudioFile(const QString& name) {
    static const QStringList extensions = {
            QStringLiteral(".mp3"), QStringLiteral(".wav"),
            QStringLiteral(".flac"), QStringLiteral(".aiff"),
            QStringLiteral(".aif"), QStringLiteral(".m4a"),
            QStringLiteral(".aac"), QStringLiteral(".ogg"),
            QStringLiteral(".opus"), QStringLiteral(".mp4"),
            QStringLiteral(".wma"),
    };
    const QString lowered = name.toCaseFolded();
...
bool isDjBrowsableVolume(const QFileInfo& device) {
    const QFileInfoList entries = QDir(device.absoluteFilePath()).entryInfoList(
            QDir::AllEntries | QDir::NoDotAndDotDot | QDir::Readable);
    for (const QFileInfo& entry : entries) {
        if (entry.isHidden() || shouldHideDjSystemEntry(entry.fileName())) {
            continue;
        }
        if (entry.isDir() || (entry.isFile() && isLikelyAudioFile(entry.fileName()))) {
            return true;
        }
    }
    return false;
...
std::vector<std::unique_ptr<TreeItem>> createRemovableDevices() {
    ...
    for (const QFileInfo& device : std::as_const(devices)) {
        ...
        // Treat a multi-partition USB drive as one DJ source. Boot, EFI and
        // recovery volumes contain no browsable music and only clutter the D2.
        if (!isDjBrowsableVolume(device)) {
            continue;
        }
        ret.push_back(std::make_unique<TreeItem>(
                device.fileName(),
...
std::vector<std::unique_ptr<TreeItem>> BrowseFeature::getChildDirectoryItems(
    ...
    foreach (QFileInfo one, all) {
        if (one.isHidden() || shouldHideDjSystemEntry(one.fileName())) {
            continue;
        }
